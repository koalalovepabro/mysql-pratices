...
def insert(firstname, lastname, email): # 입력값을 db에 추가시켜주는 함수
    try:
        # 연결
        db = conn()
        ...
        count = cursor.execute(sql, (firstname, lastname, email))  # sql실행 성공하면 count =  True(1), 실패하면 False(0)이 된다.
...
def deletebyemail(email):
    try:
        ...
        count = cursor.execute(sql, (email,))  # 입력값이 email 하나이므로, 튜플로 받기위해 괄호로 감싸고 콤마 하나 찍어줌
